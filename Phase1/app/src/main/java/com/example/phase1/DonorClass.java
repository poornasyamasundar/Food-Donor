...
public class DonorClass
{
    ...
    {
        Connection connection = ConnectionClass.getConnection();
        String statement = "Insert into DonorList ( name, address, email, phone, latitude, longitude, donortype ) Values ( '"
                +donor.getName()+"', '"+donor.getAddress()+"', '"+donor.getEmail()+"', '"+donor.getPhno()+"', '"+donor.getLatitude()+"', '"+donor.getLongitude()+"', ";
        if( donor.getType() )
        {
